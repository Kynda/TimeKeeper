...
class Time {
            ...
            foreach( $args as $key => $arg )
            {
                if( $args[$key] != '' )
                {
                    ...
                    switch ( $key )
                    {
                        ...
                        case 'where':
                            $query .= ' WHERE users_id=4 AND ' . $arg;
                            $subquery .= ' AND  users_id=4 AND ' . $arg;
                            break;
    ...
    {
        $sql = 'SELECT DISTINCT `account` FROM time WHERE users_id=4 ORDER BY `account`';
        return $this->db->fetchAll( $sql );
    }
    ...
    {
        $sql = 'SELECT DISTINCT `task` FROM time WHERE users_id=4 ORDER BY `task`';
         return $this->db->fetchAll( $sql ) ;
    }
